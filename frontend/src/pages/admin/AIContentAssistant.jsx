...
import { Sparkles, Wand2, Volume2, Play, Loader2 } from "lucide-react";
...
export default function AIContentAssistant() {
  ...
  const handleSubmit = () => {
    // Simulate saving the draft - in real app this would save to backend
    alert("Draft saved successfully! You can now convert it to voice or continue editing.");
  };
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground text-sm">
          Generate and refine campaign announcements, then convert them to voice using Google Cloud Text-to-Speech.
        </p>
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-4">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Edit the AI draft before publishing or converting to speech.</CardDescription>
          </CardHeader>
          <CardContent>
          ...
          <CardFooter className="justify-between">
            <Badge variant={draft ? "success" : "outline"}>{draft ? "Ready for voice" : "No draft yet"}</Badge>
            <Button onClick={handleSubmit} disabled={!draft} className="ml-auto">
              Save Draft
            </Button>
          </CardFooter>
        </Card>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base flex items-center gap-2">
            <Volume2 className="h-4 w-4 text-primary" /> 3. Convert to Voice — Google Cloud Text-to-Speech
          </CardTitle>
          <CardDescription>Generate an audio announcement for accessibility and emergency broadcast.</CardDescription>
        </CardHeader>
        ...
        <CardFooter className="flex items-center gap-3">
          <Button onClick={synthesize} disabled={!draft || synthesizing} variant="accent">
          ...
          {audioReady && (
            <Button variant="outline">
              <Play className="h-4 w-4 mr-2" /> Preview Audio
            </Button>
          )}
        </CardFooter>
      </Card>
