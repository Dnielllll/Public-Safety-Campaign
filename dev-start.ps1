...
if (-not (Test-Path $BackendEnvFile)) {
    if (Test-Path (Join-Path $Backend ".env.example")) {
        Copy-Item (Join-Path $Backend ".env.example") $BackendEnvFile
        Write-Host "Created $BackendEnvFile from .env.example" -ForegroundColor Yellow
        Push-Location $Backend
...
if (Test-Path $BackendEnvFile) {
    $BackendEnv = Get-Content $BackendEnvFile | Where-Object { $_ -match "^IPROG_|^SEMAPHORE_" }
}
